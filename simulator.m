...
orbit_config.semimajor_axis_m = [a_GPS, a_GLO, a_GAL, a_LEO];
orbit_config.eccentricity = [e_GPS, e_GLO, e_GAL, e_LEO];
orbit_config.inclination_rad = [i_GPS, i_GLO, i_GAL, i_LEO];
orbit_config.planes = [planes_GPS, planes_GLO, planes_GAL, planes_LEO];
orbit_config.sats_per_plane = [sats_per_plane_GPS, sats_per_plane_GLO, ...
    sats_per_plane_GAL, sats_per_plane_LEO];
orbit_config.sats_per_system = sats_per_system;
orbit_config.mean_motion_rad_s = [n_GPS, n_GLO, n_GAL, n_LEO];
...
observation_log = []; % code, phase, Doppler, C/N0, and tracking states
    ...
    for sys = 1:4
        ...
        for k = 1:size(c_sats, 1)
            ...
            if elev >= elevation_mask
                ...
                [observation, observation_state] = applySignalObservationModel( ...
                    signal_model, observation_state, error_model, c_sys_id, ...
                    c_sats(k,1), idx, elev, range, radial_vel, error_terms);
                distance_log = [distance_log; ...
                    ...
                    error_terms.orbit_error_m, error_terms.applied_orbit_error_m, ...
                    error_terms.receiver_clock_error_m, ...
                    error_terms.satellite_clock_error_m, error_terms.dcb_m, error_terms.noise_m];
                observation_log = [observation_log; ...
                    t_sim, c_sys_id, c_sats(k,1), observation.frequency_hz, ...
                    observation.wavelength_m, observation.code_m, ...
                    observation.carrier_phase_cycles, observation.doppler_hz, ...
                    observation.cn0_dbhz, observation.lli, observation.lock_time_s, ...
                    observation.ambiguity_cycles, range, radial_vel, elev, ...
                    observation.phase_bias_m, observation.phase_noise_m, ...
...
T_dist = array2table(distance_log, 'VariableNames', {'Time_s', 'System', 'Sat_ID', ...
    ...
    'Orbit_Error_m', 'Applied_Orbit_Error_m', 'Receiver_Clock_Error_m', ...
    'Satellite_Clock_Error_m', 'DCB_m', 'Noise_m'});
T_obs = array2table(observation_log, 'VariableNames', {'Time_s', 'System', 'Sat_ID', ...
    'Frequency_Hz', 'Wavelength_m', 'Code_m', 'Carrier_Phase_cycles', ...
    'Doppler_Hz', 'CN0_dBHz', 'LLI', 'LockTime_s', 'Ambiguity_cycles', ...
    'True_Range_m', 'Range_Rate_m_s', 'Elevation_deg', 'Phase_Bias_m', ...
...
[T_fin, T_rap, precise_product_model] = generatePreciseProducts( ...
    t_array, orbit_config, error_model);
sp3_metadata.start_time = rinex_metadata.start_time;
sp3_metadata.interval_s = time_step;
sp3_metadata.data_used = 'ORBIT';
sp3_metadata.coordinate_system = 'WGS84';
sp3_metadata.orbit_type = 'FIT';
sp3_metadata.agency = 'SIMU';

fin_sp3_filename = 'SIM0OPSFIN_20260010000_01D_10M_ORB.SP3';
fin_metadata = sp3_metadata;
fin_metadata.solution_type = 'FIN';
fin_metadata.accuracy_exponents = [1, 1, 1, 1];
fin_metadata.comments = [ ...
    "SIMULATED FIN PRODUCT: TRUTH ORBIT AND ACTUAL SATELLITE CLOCK", ...
    "MIXED GPS GLONASS GALILEO AND LEO SP3-D PRODUCT", ...
    "CLOCK VALUES ARE GIVEN IN MICROSECONDS IN THE P RECORDS", ...
    "FOR ALGORITHM VALIDATION; NOT AN OFFICIAL IGS PRODUCT"];
fin_sp3_summary = writeSp3Product(fin_sp3_filename, T_fin, fin_metadata);

rap_sp3_filename = 'SIM0OPSRAP_20260010000_01D_10M_ORB.SP3';
rap_metadata = sp3_metadata;
rap_metadata.solution_type = 'RAP';
rap_metadata.accuracy_exponents = [11, 12, 11, 14];
rap_metadata.comments = [ ...
    "SIMULATED RAP PRODUCT: FIN PLUS RAC ORBIT PRODUCT ERROR", ...
    "RAP CLOCK INCLUDES AN INDEPENDENT CLOCK ESTIMATION ERROR", ...
    "MIXED GPS GLONASS GALILEO AND LEO SP3-D PRODUCT", ...
    "FOR ALGORITHM VALIDATION; NOT AN OFFICIAL IGS PRODUCT"];
rap_sp3_summary = writeSp3Product(rap_sp3_filename, T_rap, rap_metadata);

save('distance_data.mat', 'T_dist', 'T_obs', 'error_model', 'signal_model', ...
    'orbit_config', 'T_fin', 'T_rap', 'precise_product_model', ...
    'rinex_metadata', 'rinex_summary', 'fin_metadata', 'rap_metadata', ...
    'fin_sp3_summary', 'rap_sp3_summary');
...
disp(['FIN SP3 product: ', char(fin_sp3_summary.filename)]);
disp(['RAP SP3 product: ', char(rap_sp3_summary.filename)]);
disp(['SP3 records per product: ', ...
    num2str(fin_sp3_summary.position_clock_records_written)]);
